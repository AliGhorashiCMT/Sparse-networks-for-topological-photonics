...
### Scripts and notebooks that make the figures in the main text and supplement
  ...
  **inverse_design_edge_states.ipynb**: Band structures for edge state dispersion in **Figure 4** of the main text. This notebook finds the symmetry eigenvalues for the PhC chosen in the main text and also saves the relevant figures in `./figures/edge_states.pdf` and `./figures/edge_state_unit_cell.pdf`. The supercell dispersion is plotted by loading the log file: `./inverse_design_two_tone_symbols_random/changing_contrast/edge_states/logs/dim2-sg2-100464-res32-tm.log`.

  **Inverse_Design_Dirac_Points.ipynb**: More in depth analysis of the Dirac point PhC of **Figure 4**. Verifies the non-trivial symmetry eigenvalues and saves the relevant figures in `./figures/dirac_point_bands.pdf`, `./figures/dirac_point_bz.pdf` and `./figures/dirac_point_ucell.pdf`. The location of the Dirac points is found by loading the file `./inverse_design_two_tone_symbols_random/changing_contrast/dirac_points/logs/dim2-sg2-101629-res64-tm.log`.
