...
id = 1622 # Index of the inverse designed PhC
real_id = id 

contrast_idxs = 1:2000 # Sweep the contrast over 200 increments
...
for uc_coef in uc_coefs
    global uc_coefs_str *= " $(uc_coef)"
end
uc_coefs_str *= ")"
inputs = inputsv[id]

for contrast_idx in task_id:num_tasks:contrast_idxs[end]
    epsout = 1 + (5/length(contrast_idxs))*contrast_idx	
    epsin = 1
    run(`./runlattice-contrasts.sh $(inputs) $(uc_coefs_str) $(uc_gvecs_str) dim2-sg$sgnum-$(real_id)-$(contrast_idx)-res64-$(mode) $(mode) $(sgnum) $(epsin) $(epsout) $(medianeps)`) 
end
