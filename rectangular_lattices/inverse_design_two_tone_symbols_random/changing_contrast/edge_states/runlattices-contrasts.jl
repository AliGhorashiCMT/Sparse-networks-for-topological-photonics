...
# since we give the fourier components later anyway and epsin is not used in any way in our code
    ...
    run(`./runlattice-contrasts.sh $(inputs) $(uc_coefs_str) $(uc_gvecs_str) dim2-sg$sgnum-$(real_id)-res32-$(mode) $(mode) $(sgnum) $(epsin) $(epsout) $(medianeps) $(contrast_idx)`) 
