...
for id in task_id:num_tasks:ids[end]
    ...
    println("\n\n")
    real_id = id
    epsin = quantile(vec(dielectric_function), 0.25)
    epsout = quantile(vec(dielectric_function), 0.75)
    run(`./runlattice.sh $(inputs) $(uc_coefs_str) $(uc_gvecs_str) dim2-sg$sgnum-$(real_id)-res64-$(mode) $(mode) $(sgnum) $(epsin) $(epsout) $(medianeps)`) 
end
